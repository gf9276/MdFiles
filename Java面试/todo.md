...
524. 如果一分钟限制只能调用60次接口，怎么用redis实现。
525. 索引失效
526. 如何分析sql语句
527. gcroots可能是哪些对象。局部变量表、静态变量、常量、native修饰、同步锁持有
528. delayqueue。
529. 集合有哪些。
530. get和post区别。
531. https原理、加密方法。RSA四次握手
532. http2.0有什么改进。头部压缩（动态表+静态表），支持并发，支持主动推送
533. jdk21的新特性。虚拟线程、switch case模式匹配（允许case类型、null）、支持ZGC、有序集合规范化、动态代理风险警告、放弃支持win32、KEM非对称加密。
534. springSecurity。
535. springcloud用了哪些组件。
536. 秒杀怎么设计。
537. 限流怎么做。
538. 令牌桶、漏桶
539. 判断2的倍数，看二进制最后一位是不是0
540. 一个接口多个实现，如何固定条件注入
541. 缓存预热？
542. 数据库数据越来越多，如何优化。
543. AOP场景举例。日志、全局统一异常处理、事务、缓存
544. 为什么非公平锁效率更高？
545. TCP网络控制，超时重传、滑动窗口、快重传。
546. 反转链表那几题都看看。
547. ArrayList扩容机制。
548. 集合有哪些。
549. 限流怎么做的（有消息队列嘛，这个算不算令牌桶）
550. mysql如何保证数据不丢失的？
551. 自旋的线程消耗CPU资源嘛？
552. explain。id 该条查询的具体id；table：查询的表；select_type：查询类别simple就是简单查询 primary就是包含了复杂的子部分；type：查询类型，all，index，range，ref，eq_ref，const；possible_key和key，可能用到的索引和实际用到的索引；key_len：索引字节数；rows：大致行数